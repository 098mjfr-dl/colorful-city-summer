...
namespace MaxIceFlameTemplate.Camera
{
	public class CameraTrigger : MonoBehaviour
	{
		...
		private void OnTriggerEnter(Collider other)
		{
			...
			if (ActivePosition)
			{
				SetCamera.DoPos = DOTween.To(() => SetCamera.AddPosition, delegate(Vector3 a)
				{
					SetCamera.AddPosition = a;
				}, NewAddPosition, NeedTime).SetEase(Ease);
			}
			if (ActiveRotate)
			{
				SetCamera.DoRot = DOTween.To(() => SetCamera.Rotate, delegate(Vector3 a)
				{
					SetCamera.Rotate = a;
				}, NewRotate, NeedTime).SetEase(Ease);
			}
			if (ActiveDistance)
			{
				SetCamera.DoDis = DOTween.To(() => SetCamera.DistanceFromObject, delegate(float a)
				{
					SetCamera.DistanceFromObject = a;
				}, NewDistance, NeedTime).SetEase(Ease);
			}
			if (ActiveSpeed)
			{
				SetCamera.DoSpe = DOTween.To(() => SetCamera.FollowSpeed, delegate(float a)
				{
					SetCamera.FollowSpeed = a;
				}, NewFollowSpeed, NeedTime).SetEase(Ease);
			}
		}
	}
}
